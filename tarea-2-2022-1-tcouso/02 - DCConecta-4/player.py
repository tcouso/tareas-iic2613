...
def random_move():
    pos = rd.randint(1, 7)
    bomb_state = rd.choice([True, False])
    if bomb_state:
        bomb_value = rd.choice(["vertical", "horizontal", "classic"])
    else:
        bomb_value = None
    bomb = [bomb_state, bomb_value]
...
class AiPlayer:
    ...
    def pick_movement(self, match): # tenemos que encontrar el error aqui
        """
        Returns a tuple (pos, bomb):
